...
%token PROGRAM DECL_LIST VAR_DECL FUNC_DECL FUNC_DEF EXT_DEC_LIST ARRAY_LIST ARRAY_DECL PARA_LIST PARA_DECL STMT_LIST COMP_STMT DEF DEF_LIST DEC DEC_LIST EXP_STMT
...
//规则定义
%%
Program : DeclList  {$$=mknode(1,PROGRAM,yylineno,$1); semantic_Analysis($$,0,"",0,' ',0);/*display($1,0);*/ /*semantic_Analysis0($1);*/}     //显示语法树,语义分析
        ; 
DeclList: {$$=NULL;}
...
VarDecl : ID    {$$=mknode(0,ID,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
        | ID ArrayList  {$$=mknode(1,ARRAY_DECL,yylineno,$2);strcpy($$->type_id,$1);} 
        ;
